...
- Optional parallel: --parallel=1 (Alias: --paralell=1) => nutzt PROZESSE (kein Threading) wegen SQLite.
- Seriell (default) lädt das Save nur einmal und führt alle Exporte nacheinander aus.
- Print-Format erweitert:  [OK][type] Wrote  XXXX entries in YY.yy secs -> <pfad>
- Start-Priorität (sofern gesetzt): tamed zuerst, dann structures, danach die restlichen in ursprünglicher Reihenfolge.

Beispiele:
  py asv_json_export/export_json.py --type="all" --serverkey="test1" --savegame=".../Astraeos_WP.ark" --output=../output
  py asv_json_export/export_json.py --type="players,tamed" --parallel=1 --serverkey="test1" --savegame=".../Astraeos_WP.ark" --output=../output
"""
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    # Parallel-Flag (mit Tippfehler-Alias)
    p.add_argument("--parallel", type=int, default=0, choices=(0, 1), help="0=seriell (Save einmal laden), 1=parallel (Prozesse)")
    p.add_argument("--paralell", dest="parallel", type=int, choices=(0, 1), help=argparse.SUPPRESS)
...
def export_tamed(save: AsaSave, export_folder: Path, save_path: Path) -> Tuple[str, int]:
    ...
    for dino_id, dino in dino_api.get_all_tamed(True).items():
...
def _run_one_in_process(t: str, save_path: Path, export_folder: Path, max_level: int, max_level_bionic: int) -> Tuple[str, int, str, float]:
    """
    Läuft in einem separaten Prozess. Öffnet eigenes AsaSave (SQLite-Conn im selben Prozess).
    Gibt zusätzlich die Dauer in Sekunden zurück.
    """
    t0 = time()
    ...
    if t == "players":
        ...
        fname, cnt = export_tamed(save, export_folder, save_path)
    elif t == "wild":
...
def main() -> None:
    ...
    requested_raw = parse_types(args.type)
    requested = prioritize_types(requested_raw)
    export_folder = ensure_export_folder(args.output, args.serverkey)
    save_path = args.savegame  # CLI-Pfad weiterreichen

    results: List[Tuple[str, int]] = []

    # Parallel mit Prozessen (jeder Prozess lädt eigenes Save → SQLite safe)
    do_parallel = args.parallel == 1 and len(requested) > 1
    if do_parallel:
        # Reihenfolge der Submits entspricht der Priorisierung
        with ProcessPoolExecutor(max_workers=len(requested)) as ex:
            futures = [
                ex.submit(_run_one_in_process, t, save_path, export_folder, args.max_level, args.max_level_bionic)
                for t in requested
            ]
            for fut in as_completed(futures):
                try:
                    fname, cnt, t, elapsed = fut.result()
                    results.append((fname, cnt))
                    print(f"[OK][{t}] Wrote {cnt:>6} entries in {elapsed:.2f} secs -> {export_folder / fname}")
                except Exception as e:
                    print(f"[ERR] {e}")
    else:
        # Seriell: Save einmal laden und wiederverwenden
        save = AsaSave(save_path)
        for t in requested:
            t0 = time()
            if t == "players":
                ...
                res = export_tamed(save, export_folder, save_path)
            elif t == "wild":
    ...
    print(f"Finished {len(results)} export(s), total entries: {total}, runtime: {took:.2f}s")
