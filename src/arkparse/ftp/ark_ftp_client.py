...
class ArkFtpClient:
    ...
    def get_file_contents(self, remote_file) -> bytes:
        contents = b""
        self.ftp.retrbinary('RETR ' + remote_file, contents.join)
        return contents
    ...
    def download_tribe_file(self, file_name, output_directory, map: dict = None) -> Path:
        self.nav_to_save_files(self._check_map(map))
        local_file = output_directory / file_name
        self.download_file(file_name, local_file)
        return local_file
    
    def download_profile_file(self, file_name, output_directory, map: dict = None) -> Path:
        self.nav_to_save_files(self._check_map(map))
        local_file = output_directory / file_name
        self.download_file(file_name, local_file)
        return local_file

    def download_save_file(self, file_name, output_directory, map: dict = None):
        self.nav_to_save_files(self._check_map(map))
        local_file = output_directory / file_name
        self.download_file(file_name, local_file)
        return local_file
