...
class ByteOperator(BaseValueParser):
    ...
    def replace_bytes(self, position: int, new_bytes: bytes, nr_to_replace: int = None):
        if nr_to_replace is None:
            nr_to_replace = len(new_bytes)
        self.byte_buffer = self.byte_buffer[:position] + new_bytes + self.byte_buffer[position + nr_to_replace:]
